...
class InsertionSortTest < Minitest::Test
  context 'insertion sort' do
    ...
    should 'sort a simple unordered array' do
      ...
      assert_equal @arrbig.sort, @sorter.insertion_sort(@arrbig)
    end

    should 'sort a sorted array' do
      assert_equal @arrsorted.sort, @sorter.insertion_sort(@arrsorted)
    end

    should 'return an empty array' do
      assert_equal @arr0.sort, @sorter.insertion_sort(@arr0)
    end

    should 'return an array with 1 element' do
      assert_equal @arr1.sort, @sorter.insertion_sort(@arr1)
    end

    should 'sort an unsorted array with 2 element' do
      assert_equal @arr2.sort, @sorter.insertion_sort(@arr2)
    end

    should 'sort an array which is reverse sorted' do
      assert_equal @reverse.sort, @sorter.insertion_sort(@reverse)
    end

    meta current: true
    should 'sort an array of negative numbers' do
        # binding.pry
      assert_equal @negative.sort, @sorter.insertion_sort(@negative)
    end

    should 'sort an array of chars' do

      assert_equal @arrchar.sort, @sorter.insertion_sort(@arrchar)
    end
  end
 end
